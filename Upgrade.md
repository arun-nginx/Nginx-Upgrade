NGINX Plus Upgrade Plan: R32→R37 (VM) & R35→R37 (Containers)
⚠️ CRITICAL NOTE — R32 to R33+ is a BREAKING CHANGE: Starting with NGINX Plus R33, a JWT (JSON Web Token) license is mandatory. Without it, NGINX Plus will not start after upgrade. This is the most important consideration for the VM upgrade path. This requirement does not apply to the R35→R37 container path (JWT already in use).
...
AlmaLinux	8, 9
Alpine Linux	3.19+
⚠️ Ubuntu 20.04 is removed as of R35. If running Ubuntu 20.04, an OS upgrade is required first.
...
Log in to MyF5 Portal
Go to My Products & Plans > Subscriptions
Find your NGINX subscription → Select Subscription ID
Download the JSON Web Token (JWT) file
Place it on each NGINX Plus instance:
sudo mkdir -p /etc/nginx
sudo cp license.jwt /etc/nginx/license.jwt
sudo chmod 644 /etc/nginx/license.jwt
Validate the JWT has no trailing spaces/newlines:
xxd /etc/nginx/license.jwt | tail -3
# Ensure no "20" (space) or "0a" (newline) at end of file
Per KB K000148865: NGINX Plus R33 Upgrade - JWT License Location: The JWT must be in the same directory as the nginx.conf (default: /etc/nginx/license.jwt). If using a custom nginx.conf path, place it in that same directory.

1.4 Network / Connectivity Check (R33+ Requirement)
R33+ requires outbound HTTPS (TCP 443) for usage reporting.

Internet-connected: Allow outbound to product.connect.nginx.com on port 443
Disconnected/Air-gapped: Configure NIM as the usage reporting endpoint:
# Add to nginx.conf main block (AFTER upgrade)
mgmt {
    usage_report endpoint=<NIM-FQDN>;
}
Per KB K000153170: Failure to configure usage reporting in disconnected environments will result in 503 errors and NGINX stopping traffic processing.

Grace Period: If the instance has reported at least once, a 180-day grace period applies during outages. Per KB K000148234 Internal

1.5 NIM Compatibility Check (If Managed by NIM)
Per NIM documentation: NGINX Plus R33+ requires NGINX Instance Manager 2.18 or later.
...
Per KB K000138256: Direct multi-version upgrades are supported, but you must review the "Important Changes in Behavior" for each intermediate release (R33, R34, R35, R36, R37).
...
The container upgrade path is significantly simpler than the VM path since:

JWT is already in use (R35 already requires it)
No in-place package upgrade — containers are replaced with new images
The recommended approach is blue/green deployment (new container alongside old)
...
Check for any breaking changes between R35 and R37 at NGINX Plus Releases.
...
Update your load balancer or DNS to point to the new R37 container
...
Go to Instances → Filter by Availability = Unavailable
Select old instances → Delete selected
Per Lab 5: Upgrade NGINX Plus to the latest version
...
📚 Key References
KB K000148681: NGINX Plus was NOT Upgraded — JWT missing error
KB K000148865: JWT License Location — JWT file placement
KB K000148775: Invalid License Token — Trailing chars in JWT
KB K000153170: 503 After Upgrade in Disconnected Env — mgmt block config
KB K000153173: How to View Usage Report — Verify reporting
KB K000159973: AWS Marketplace Upgrade — AWS-specific note
NGINX Plus Releases — Release dates & EoSD policy
Lab 5: Upgrade NGINX Plus — Docker & VM upgrade lab